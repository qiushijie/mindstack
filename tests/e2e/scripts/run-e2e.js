...
async function startWailsDev() {
  ...
  // Expose config dir to Playwright subprocess as well
  process.env.MINDSTACK_CONFIG_DIR = configDir
...
async function main() {
  ...
  if (!existing) {
    ...
    // Even when reusing an existing wails dev process, give Playwright an isolated
    // config directory so tests never read from or write to the user's real config.
    const configDir = createTempDir()
    process.env.MINDSTACK_CONFIG_DIR = configDir
    const seedConfig = JSON.stringify({
      settings: { locale: 'zh', theme: 'light', autoSave: true, autoSaveDelay: 5 },
    ...
    fs.writeFileSync(path.join(configDir, 'config.json'), seedConfig)
    console.log('[e2e] Using existing wails dev process with isolated config dir:', configDir)
  }
