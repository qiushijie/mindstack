...
export const addColumnRightCommand: EditorCommand<TableCommandPayload> = {
  id: 'editor.table.addColumnRight',
  ...
  execute(ctx: CommandContext, payload: TableCommandPayload): CommandResult {
    ...
    newHeaders.splice((colIdx ?? 0) + 1, 0, '')

    const newRows = rows.map(row => {
      const newRow = row.map(c => c.content)
      newRow.splice((colIdx ?? 0) + 1, 0, '')
      return newRow
    })
