	...
	go func() {
		...
		for {
			...
			if chunk.Message.Content != "" && !e.hasToolCall.Load() {
				...
				runtime.EventsEmit(e.ctx, "chat:message:chunk", string(data))
			}
		}
	}()
...
func (s *Service) runUnifiedAgent(sessionID uint, messages []*einoschema.Message, req ChatRequest) {
	runtime.LogInfof(s.ctx, "[chat] starting unified agent for session=%d", sessionID)

	chatModel := s.llm.GetToolCallingModel()
	if chatModel == nil {
		runtime.LogError(s.ctx, "[chat] model does not support tool calling, falling back to plain chat")
	...
	runtime.LogInfof(s.ctx, "[chat] creating ReAct agent with edit tools")
	...
	if err != nil {
		runtime.LogErrorf(s.ctx, "[chat] failed to create agent: %v", err)
		data, _ := json.Marshal(map[string]any{
			"type":      "chat",
			"sessionId": sessionID,
			"done":      true,
		...
		runtime.EventsEmit(s.ctx, "chat:message:chunk", string(data))
		return
	}
	...
	runtime.LogInfof(s.ctx, "[chat] calling agent.Generate with %d messages", len(messages))
	resp, err := agent_.Generate(s.ctx, messages, agent.WithComposeOptions(compose.WithCallbacks(emitter)))
	if err != nil {
		runtime.LogErrorf(s.ctx, "[chat] agent.Generate error: %v", err)
		data, _ := json.Marshal(map[string]any{
			"type":      "chat",
			"sessionId": sessionID,
			"done":      true,
	...
	runtime.LogInfof(s.ctx, "[chat] agent response: role=%s, contentLen=%d, hasToolCall=%v", resp.Role, len(resp.Content), emitter.hasToolCall.Load())
	...
	runtime.LogInfof(s.ctx, "[chat] content=%s, isEdit=%v", resp.Content, isEdit)

	if isEdit {
		runtime.LogInfof(s.ctx, "[chat] detected edit result, changes=%d", len(toolResult.Changes))
		...
		runtime.EventsEmit(s.ctx, "chat:edit:chunk", string(data))
		...
		runtime.EventsEmit(s.ctx, "chat:message:chunk", string(doneData))
		...
		if _, err := s.store.AddMessage(sessionID, "assistant", contentToSave); err != nil {
			runtime.LogErrorf(s.ctx, "failed to save assistant message: %v", err)
		}
	} else {
