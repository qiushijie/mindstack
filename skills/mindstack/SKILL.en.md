...
Show document metadata. The path must be an **absolute path** (e.g., the `path` returned by `doc ls` or `search`) and the file must exist. Metadata is generated by `sync` via the LLM.
...
- **`doc meta` `found: false`**: exit 0 is not an error; it means the document has not been synced yet, run `sync` to generate metadata.
